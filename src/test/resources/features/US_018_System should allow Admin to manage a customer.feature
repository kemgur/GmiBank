Feature: US_018 System should allow Admin to manage a customer
  ...
  Background:
    ...
    And user sends valid manager "manager_username" to username textbox
    And user enters valid manager "manager_password" in the password textbox
  ...
  Scenario: TC_004_There should be a View option where customer can be navigated to all customer info and see
  edit button there

    And user click on the first customer's View button
    And there should be Edit button at the bottom and left

  Scenario: TC_005_There should be an Edit button where all customer information can be populated
    And there should be Edit button  of first customer on the page that opens
...
@hs
    Scenario: TC_007_User can delete a customer, but seeing a message if the user is sure about deletion
      ...
      Then user should see "delete_message" at the and
